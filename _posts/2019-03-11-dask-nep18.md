...
need to call the `.compute()` function on result arrays to actually compute
them.
...
We will now talk a bit about potential applications of the ``__array_function``
...
To verify how Dask-GLM with Dask arrays would compare with CuPy arrays, we also
did some linear regression benchmarking of Dask-GLM solvers. The results below
were obtained from a training dataset with 1000 features of 100 dimensions,
and 1000 test features.
...
orders of magnitude faster using CuPy as a backend for reasonably large
problems, compared to using Dask as a backend on a single node. Note also that
CuPy is running on a single GPU.
