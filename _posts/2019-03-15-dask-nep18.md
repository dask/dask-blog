...
<img src="https://gist.githubusercontent.com/pentschev/86445c9c89c3b956f2289d0e9e0c9953/raw/a2814d7bcd4066257bf1c75c9e16f8fc541feec4/linreg.png">
...
way we are handling this issue consists in adding a new attribute ``_meta`` to
Dask ``Array`` in substitution to the simple ``dtype`` that currently exists.
This new attribute will not only hold the ``dtype`` information, but also an
empty array of the backend type used to create the ``Array`` in the first
place, thus allowing us to internally reconstruct arrays of the backend type,
without having to know explicitly whether it's a NumPy, CuPy, Sparse or any
other NumPy-like array. For additional details, please refer to [Dask Issue
