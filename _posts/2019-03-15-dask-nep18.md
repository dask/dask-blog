...
needs to call the proper library dispatcher in addition to passing the correct array,
which would incur in finding all NumPy calls in the code and replacing those by
the correct library's function call, depending on the input array type. After
``__array_function__``, the same NumPy function can be called, using the Dask
array ``dx`` as input:
