...
**TLDR; cuML is fast single GPU ML -- slow but functional for distributed GPU**
...
optimizations.  Naively, we thought, "well, we have GPU machine learning, we have distributed hyper parameterization...
...
Again, reminding ourselves that the data is small ~28KB, we don't expect to observe cuml performing faster than sklearn. Instead, we want to demonstrate functionality.  Fortunately, dask-ml GridSearchCV adheres to the same API as sklearn; we can swap out GridsearchCV from sklearn and use dask-ml's which will use all of the GPUs we have available.
...
cuml + distributed GridSearchCV does significantly *worse* as data sizes increase!  Why?  Primarily, two reasons:
...
There are 50 (cv=5 times 10 parameters for alpha) instances of chunking up our test data set and scoring performance.  That means 50 times we are moving data back forth between host and device for fitting and 50 times for scoring.

## Immediate Future Work

However!  We know the problems, GH Issues have been filed, ware working on these issues -- come help!
