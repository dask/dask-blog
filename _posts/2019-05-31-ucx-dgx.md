...
improvements in setting up Dask in heterogeneous situations targetting
...
Communication also increasingly becomes a bottlenck when our computation
...
MPI, or with very custom codes.  We were trying to get close to the
communicatino bandwidth of MPI while still maintaining the ease of
...
Akshay gave a great talk about UCX at the GPU Technology Conference 2019.
...
when the proper hardware is present though other bottlenecks, like
serialization when dealing with text or JSON-like data, may occur
...
[NCCL](https://developer.nvidia.com/nccl), which knows to route things
correctly, but is something of a challenge for other more general purpose
systems like Dask to adapt to (or at least, I found it a challenge).
...
-   **Routing within complex networks**:
    ...
    3.  Avoiding complex networks altogether.  Apparently newer systems like
        the DGX-2 use NVSwitch, which is more uniform.
...
```
conda install ipykernel jupyterlab nb_conda_kernels
```
