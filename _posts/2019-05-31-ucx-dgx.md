...
The benefit of using a DGX-optimized configuration is clearly demonstrated by the performance of the [SVD Dask-GPU](https://github.com/mrocklin/dask-gpu-benchmarks/blob/master/cupy-svd.ipynb) benchmark. By swapping the usual `LocalCUDACluster` definition with a `DGX` cluster definition (in the first cell), the overall communication time is reduced by almost an order of magnitude. To produce the task-stream figures below, the benchmark was run on a DGX-1 with `CUDA_VISIBLE_DEVICES=[0,1,2,3]`.  It is clear that the red task bars, corresponding to inter-process communication, are significantly compressed. 

**DGX-Optimized Cluster**:
<iframe src="../images/task_stream_dgx_dgx16.html" width="800" height="400"></iframe>
        
**Default LocalCUDACluster**:
<iframe src="../images/task_stream_lcc_dgx16.html" width="800" height="400"></iframe>
