...
Instead, we would have wanted to write down the following:
...
1.  **Convert to and from ITK Image objects**: ITK functions don't consume and
    ...
    time.  See [ITK
    #1136](https://github.com/InsightSoftwareConsortium/ITK/issues/1136) for a
    feature request.
...
2.  **Unpack and pack singleton dimensions**: Our Dask arrays have shapes like
    ...
    So our `map_blocks` function gets NumPy arrays of size `(1, 1, 201, 1024, 768)`.
    ...
    And then when we're done, our array expects to get back 5d arrays of
    roughly the same shape, so we add them back in
    ...
    slicing syntax, but does need to be done each time, and so presents some
    friction.
...
You would swap out `LocalCluster` with one of the
[other options](https://docs.dask.org/en/latest/setup.html).
...
```python
def richardson_lucy_deconvolution(img, psf, iterations=1):
    import itk   # <--- this is how we work around this problem
```
...
`map_blocks` immediately without any packing/unpacking

```python
import skimage.restoration

out = da.map_blocks(skimage.restoration.richardson_lucy, imgs, psf)
```
...
Regardless, our our goal in this experiment was to see how well ITK and Dask
...
interviewed said that they wanted faster deconvolution.  Now we know why.  It's
both very common, and *very slow*.

Running deconvolution on a single chunk takes around 2-4 minutes, and we have
hundreds of chunks.  Multi-core parallelism can help a bit here, but this
problem may also be ripe for GPU acceleration.  Similar operations typically
have 100x speedups on GPUs.  This might be a more pragmatic solution than
scaling out to large distributed clusters.
