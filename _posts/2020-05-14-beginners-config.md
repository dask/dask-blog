...
If you have tested your flows locally, a simple heuristic is to multiply the CPU, storage, and memory usage of your flows by some multiplier that is related to how scaled down your local flow run is from your expected production usage. For example, if you test your flows locally with a 10% sample of data, multiplying any observed resource usage by at least 10 will get you close to your minimum instance size. Doing this for all flows that will be deployed, taking into account flow schedules that may cause flows to stagger in their concurrency, is a good first pass technique.
