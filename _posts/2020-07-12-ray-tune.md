...
Dask-ML's model selection module has every one of the features. Briefly,
Dask-ML has the following features:

* **Consistency with Scikit-Learn API:** Dask-ML is consistent with the
  Scikit-Learn API.
* **Modern hyperparameter tuning techniques:** Dask-ML offers state-of-the-art
  hyperparameter tuning techniques in it's Scikit-Learn interface.
...
Now, let's show how to obtain each of the 5 features above in Dask-ML. To do
that, let's look at each feature and see what Dask-ML has to offer.
...
*Dask-ML model selection supports libraries many libraries including Scikit-Learn, PyTorch, Keras, LightGBM and XGBoost.*
...
These are configured to perform the same task: sample 100 parameters and train
for no longer than 100 "epochs" or passes through the data. Each estimator is
set up as their respective documentation suggests;[^random-search] the complete
setup is in [the appendix](#appendix).

Each search uses 8 workers with a single cross validation split, and a
`partial_fit` call takes one second with 50,000 examples. Here's how long each
library takes to complete this search:
...
1. **A Bayesian sampling scheme for the Hyperband implementation** that's
   similar to BOHB's ([dask-ml#697][697]).
