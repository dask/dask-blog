...
This claim has a misconception: Dask-ML has provided access to "cutting edge
hyperparameter tuning techniques" with a Scikit-learn compatible API for over a
year. This post will provide facts that correct the misconception Ray stated.
To do that, let's start by looking at the features of Ray's tune-sklearn:
...
this drastic improvement persists in follow-up works.[^follow-up]
...
and memory.[^openai] Currently, this implementation is completely separate from
Dask-ML's Hyperband implementation -- naturally it's future work to combine the
two.
