...
*This post also lives at http://stsievert.com/TODO (with better styling)*
...
given at model initialization. An example is with the `n_components` parameter
in Scikit-learn's [TSNE] which determines the dimension to embed in. For a more
...
[^cores]:Scikit-learn does support running `n_jobs` models in parallel on a single machine with the `n_jobs` keyword.
...
features. Other implementations that don't follow the Scikit-learn interface
...
API. To correct this statement, let's look at the features of Ray's
tune-sklearn:
...
search = RandomizedSearchCV(clf, params, n_iter=20)
search.fit(X, y)
...
search = TuneSearchCV(clf, params, early_stopping=True)
search.fit(X, y, classes=[0, 1])
```
...
There are two methods to avoid unnecessary work in hyperparameter optimization:

1. Stopping poor performing models.
...
Dask-ML has an implementation of "Hyperband," an early stopping algorithm.  At
the most basic level, Hyperband is an "early stopping" technique for
Scikit-learn's `RandomizedSearchCV`: it stops low performing at certain points
in the training process.[^stopping]
...
[Dask-ML][hscv], [keras-tune], [Optuna], [AutoML],[^automl] and [Microsoft's
NNI][nni]. The original paper shows a rather drastic improvement over all the
relevant implementations,[^hyperband-figs] and this drastic improvement
persists in follow-up works.[^follow-up] Some illustrative results from Hyperband are below (which are Figures 7 and 8 in the [Hyperband paper][hyperband-paper]):
...
black box's quality. This is difficult in a distributed system because if there
were an infinite number of workers, the Bayesian sampling algorithm wouldn't
have time to deduce anything after initializing models.
...
Hyperband's parallel architecture.[^scaling]
...
Ray tune-sklearn supports these frameworks:
...
<sup>\* Only if the model implements `partial_fit`</sup><br>
<sup>\*\* Due to recent improvements (which are detailed in [the conclusion]) and were motivated by this post.</sup>
...
Ray's tune-sklearn is fast, at least in some benchmarks in [the
introduction][1] which are run against Scikit-learn and Dask-ML's
`GridSearchCV`. A more relevant benchmark would be against Dask-ML's
`HyperbandSearchCV` because it is very similar to the algorithm in Ray's
tune-sklearn.

Let's run that relevant benchmark and compare the following implementations:

* Scikit-learn's `RandomizedSearchCV`
* Ray's `TuneSearchCV`, configured as suggested in the documentation.
* Dask-ML's `HyperbandSearchCV`, configured as suggested in the documentation.
...
3. **Adding support for more libraries, including Keras**
   ([dask/dask-ml#696][696], [dask/dask-ml#699][699]). We have already adding
   a `partial_fit` implementation to a Scikit-learn wrapper for Keras, SciKeras
