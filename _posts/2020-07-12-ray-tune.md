...
Dask-ML avoid unnecessary work for hyperparameter optimization. The easiest
method to avoid unnecessary work is to randomly initialize models and stop
poor-performing models early. For example, if half the models are stopped
halfway through training then 25% less work will be performed. Dask-ML has an
implementation of a popular principled early stopping called "Hyperband."
Functionally, Hyperband and Scikit-learn's `RandomizedSearchCV` are the same
except Hyperband makes decisions on when to stop models.[^stopping]
