...
The state-of-the-art in hyperparameter optimization is currently the popular
algorithm called "[Hyperband][hyperband-paper]." Hyperband saves on total
computation cost via a principled early stopping scheme; past that, it's the
same as Scikit-Learn's popular `RandomizedSearchCV`.
...
*Dask-ML and Ray are much faster than Scikit-Learn.*
...
asynchronous variant of Dask-ML's Hyperband; however, Dask-ML can scale up to 24
workers for the search in "[Better and faster hyperparameter optimization with
...
This benchmark is only concerned with showing the time to complete
`search.fit`. It ignores some relevant facts:

* Dask-ML's model selection framework prioritizes fitting high scoring models,
  which is especially relevant in the serial context.[^priority] If there are
  25 models to fit and only 5 slots to run them in, Dask-ML selects the models
  that have the highest score. See "[Better and faster hyperparameter
  optimization with Dask][db-bf]" for benchmarks.
* Ray's implementation performs more computation because it implements the
  asynchronous variant of Hyperband, or it performs more floating-point
  operations.
...
3. **Adding support for more libraries, including Keras**
   ([dask/dask-ml#696][696], [dask/dask-ml#699][699],
   [dask/dask-ml#713][713]). We have already added a `partial_fit`
   implementation to SciKeras, a Scikit-Learn wrapper for Keras in
   [scikeras#17], and are closely following [tensorflow#39609].
