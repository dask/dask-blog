...
Here's how to use Dask-ML's hyperparameter optimization:
...
## Modern hypereparameter tuning technique

Ray's tune-sklearn provides access to advanced hyperparameter optimization:
...
Ray's tune-sklearn is fast, at least in some benchmarks with Scikit-learn and
Dask-ML's `GridSearchCV`. A more relevant benchmark would be against Dask-ML's
`HyperbandSearchCV` because both Ray and Dask-ML have "adaptive" model
selection algorithms that *adapt* to previous calls to `model.fit` to choose
