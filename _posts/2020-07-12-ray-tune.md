...
can't be learned from data. These "hyperparameters" have to be given at model
initialization. An example is with the `n_components` parameter in
Scikit-learn's [TSNE], which determines the dimension to embed into. For a more
complete overview, see "[How to Use t-SNE Effectively][tsne]" or "[Tuning the
hyper-parameters of an estimator][tuning]."

Scikit-learn provides some basic and popular implementations in their [model
selection module][skl-ms], `RandomizedSearchCV` and `GridSearchCV`. These
implementations are widely used, especially when the data fits in memory and
the model fitting happens pretty quickly (less than minutes). However, a more
advanced implementation is required when the computation takes longer or the
data becomes large.[^cores]

[^cores]:Scikit-learn does have some support for longer training times with their Joblib backend via `n_jobs`.

There's a host of libraries and frameworks out there to address this problem.
Scikit-learn's interface has been mirrored [in Dask-ML][dml-ms] and
...
API. To correct their statement, let's see how Dask-ML compares to each of the
features that tune-sklearn provides:
...
*Dask-ML is also consistent with the Scikit-learn API.*
...
search = TuneSearchCV(clf, params, ...)
search.fit(X, y, classes=[0, 1])
```
...
### Modern hypereparameter tuning techniques

Ray's tune-sklearn provides access to advanced hyperparameter optimization, and
[the introduction][1] made this claim:
...
Methods to avoid unnecessary work in hyperparameter optimization include the
following include the following:

1. Stopping poor scoring models before they complete training.
2. Using prior model training to figure out the best set of hyperparameters.

Dask-ML focuses on the first option because it's easier to parallelize.
Dask-ML has an implementation of "Hyperband," an early stopping technique for
Scikit-learn's `RandomizedSearchCV`. Hyperband and `RandomizedSearchCV` are the
same, except Hyperband makes decisions on when to stop models.[^stopping]

Hyperband is popular because it works.  After the introduction of Hyperband in
2016 by Li et. al, [the paper][hyperband-paper] has been cited [over 470
times][470] and has been implemented in many different libraries including
[Dask-ML][hscv], [Ray Tune][rt-hb], [Ray's tune-sklearn][rts-hb], [keras-tune],
[Optuna], [AutoML],[^automl] and [Microsoft's NNI][nni]. The original paper
shows a rather drastic improvement over all the relevant
implementations,[^hyperband-figs] and this drastic improvement persists in
follow-up works.[^follow-up] Some illustrative results from Hyperband are below
(which are Figures 7 and 8 in the [Hyperband paper][hyperband-paper]):
...
<sup>"hyperband (finite)" is similar Dask-ML's default implementation, and
"bracket s=4" is similar to Ray's default implementation. SMAC,[^smac]
spearmint,[^spearmint] and TPE[^tpe] are popular Bayesian algorithms.
"random" is random search, and "random 2x" does twice as much
computation.</sup>
</div>
...
The second method of avoiding unnecessary work is called "Bayesian
optimization." These are more difficult in any distributed system because of
...
black box's quality. This is difficult in any distributed system because if
there were an infinite number of workers, the Bayesian sampling algorithm
won't have time to deduce anything after initializing models.

Notably, Bayesian optimization algorithms have been adapted the Hyperband
algorithm. For example, AutoML's [BOHB] uses TPE and Hyperband in tandem.  Even
better, they have an intelligent way of scaling to distributed systems given
Hyperband's parallel architecture[^scaling]<sup>,</sup>[^ray-bayesian] -- it's
future to integrate this Bayesian sampling into Dask-ML.

[^ray-bayesian]:It appears Ray has preliminary Bayesian sampling and Hyperband too.
...
and memory.[^openai] Currently, Dask-ML's implementation to avoid repeated work
is completely separate from Dask-ML's Hyperband implementation -- naturally
it's future work to combine the two.

Dask-ML certainly has "cutting edge" hyperparameter optimization techniques --
it effectively avoids unnecessary work. In addition, the implementaiton behind
Dask-ML's model selection is pretty flexible. That's great news for me as as a
computer science graduate student and the improvements I'd like to see.
...
*Dask-ML model selection also supports libraries like Scikit-learn, PyTorch, Keras, LightGBM and XGBoost.*
...
<sup>\*\* Shortly; [dask/dask-ml#699][699] is a work in progress by the Dask developers that is
motivated by this post.</sup>
...
*Dask-ML also supports distributed tuning (how could it not?). In addition,
it also supports out-of-memory data.*
...
Ray's tune-sklearn runs some benchmarks in [the introduction][1] with
Scikit-learn and Dask-ML's `GridSearchCV`. A more fair benchmark would be use
Dask-ML's `HyperbandSearchCV` because it is very similar to the algorithm in
Ray's tune-sklearn. To be specific, I'm interested in comparing these methods:
...
* Ray tune-sklearn's `TuneSearchCV`. This is a slightly different early
  stopping technique for `RandomizedSearchCV`.

Each of these will be configured to sample `n_params` parameters and to train
models no longer than `max_iter` "epochs" or passes through the data.  All of
these estimators will be set up as their respective documentation suggests, and
I choose to benchmark random searches instead of grid searches because random
searches produce better results.[^random-search]

[^random-search]:Because grid searches require estimating how important each parameter is; for more detail see "[Random Search for Hyperparameter Optimization][rsho]" by Bergstra and Bengio.

[rsho]:http://www.jmlr.org/papers/volume13/bergstra12a/bergstra12a.pdf

I'm most interested in how effective the early stopping is for Ray and Dask-ML.
The parameters/data aren't interesting for this comparison; all three methods
initialize models with random selection. So, I will measure the time taken to
complete `search.fit(X, y)` for various searches. That means it the data and
model don't matter. Let's create a dummy model that takes 1 second for a
`partial_fit` call with 50,000 examples:
...
X_train, y_train = ...  # dummy data with 60,000 examples
```
...
Let's configure our searches to use 8 workers:
...
search.fit(X_train, y_train)  # 10.44 minutes
```

This search takes 10.44 minutes, a fairly long time to train with a model that
takes 2 minutes to train. Let's try to beat that with Dask-ML and Ray's
tune-sklearn. As mentioned [above](#consistency-with-the-scikit-learn-api),
both Dask-ML and Ray have Scikit-learn compatible model selection API:
...
dask_search.fit(X_train, y_train)  # 1.79 minutes
ray_search.fit(X_train, y_train)  # 1.55 minutes
```
...
with Scikit-learn. Even relatively, Dask-ML is only 15% slower than Ray with 8
workers. Ray might perform better with more workers because they implement an
asychronous variant of Dask-ML's Hyperband; however, Dask-ML can scale up to 24
workers for a search in "[Better and faster hyperparameter optimization with
Dask][db-bf]." That's the number of GPUs on 4 nodes on the [Summit
supercomputer][summit]!

[summit]:https://en.wikipedia.org/wiki/Summit_(supercomputer)#Design

The benchmark above ignores the fact that the Dask-ML implementation
prioritizes fitting high scoring models, which is especially relevant in the
serial context.[^priority] If there are 25 models to fit and only 5 slots to
run them in, Dask-ML selects the models that have the highest score. See
"[Better and faster hyperparameter optimization with Dask][db-bf]" for
benchmarks.
...
3. **Adding support for more libraries, including Keras**
   ([dask/dask-ml#696][696], [dask/dask-ml#699][699]). We have already adding a
   `partial_fit` implementation to SciKeras, a Scikit-learn wrapper for Keras
   in [scikeras#17], and are following [tensorflow#39609].
